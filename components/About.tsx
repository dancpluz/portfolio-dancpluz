...
function Technologies() {
  ...
  const { xTranslation, setMustFinish, setDuration } = useHorizontalScroll(
    FAST,
    width
  );
          ...
          {Array(16)
            ...
            .map((_, i) => (
              <MySkeleton
                ...
                containerClassName='size-16 rounded-md relative overflow-hidden relative justify-center items-center flex border border-foreground'
              />
            ))}
        </motion.ul>
      </div>
        ...
        {[...data, ...data].map((tech, i) => (
          <MyPopup
            ...
            trigger={
              <li className='w-16'>
                <Image
                  src={buildImageUrl(tech, tech.icon)}
                  width={64}
                  height={64}
                  alt={'Ícone ' + tech.alt}
                />
...
function ExperienceItem({
  ...
  experience: ExperienceResponse<IconExpand>;
}) {
            ...
            <Link href={expand.icon_ref.link} target='_blank'>
              <div className='relative size-10 lg:size-14'>
                <Image
                  ...
                  src={buildImageUrl(expand.icon_ref, expand.icon_ref.icon)}
                  fill
                  alt={'Ícone ' + expand.icon_ref.alt}
                />
              </div>
            </Link>
          ) : (
      ...
      >
        <div className='flex flex-col'>
          ...
          <span className='text-sm tracking-wider'>
            {formatTimeDifference(start_date, end_date)} |{' '}
    ...
    <div className='flex flex-col gap-16 py-16'>
      <div className='flex flex-col md:flex-row gap-4 md:gap-8 px-4 md:px-40'>
        <div className='relative overflow-hidden aspect-7/8 md:aspect-5/8 w-full md:w-[800px] rounded-3xl'>
          ...
          />
          {!isLoaded && <MySkeleton className='h-full w-full' />}
        ...
        <div className='flex gap-4 py-4 md:py-0 flex-col grow'>
          <div className='md:pr-10 pr-6 relative'>
            ...
            <div className='absolute top-1/2 -translate-y-1/2 right-0 size-16 md:size-20'>
              <MyPopup
                on='hover'
                trigger={<Image alt={'Logo Lumentosh'} src={'logo.svg'} fill />}
                position='top center'
              >
                <div className='flex flex-col'>
          ...
          <p className='md:text-3xl text-xl'>
            Inspirado pela moda, o bom design e a crescente vontade de
            solucionar problemas, me esforço para tornar a vida das pessoas mais
            fácil e expressar minha arte ao mesmo tempo.
          </p>
          <Experience />
        </div>
          ...
          <MyPopup
            ...
            trigger={
              <div className='flex flex-col gap-1 items-center'>
                <p className='whitespace-nowrap font-normal hidden md:block'>
                  CIC - UNB
                </p>
                <Image
            ...
            {Array(8)
              ...
              .map((value) => {
                ...
                if (value > semester) {
                  return (
                    <div
                      className='flex lg:flex-col flex-row items-center gap-1.5 grow'
                      key={value + 'º Semestre'}
                    >
                      <MyPopup
                        on='hover'
                        trigger={
                          <div className='size-3 rounded-full border-2 border-alternate' />
                        }
                      ...
                      trigger={
                        <div
                          className={`w-3 h-3 rounded-full bg-foreground ${
                            value === semester
                              ? 'before:animate-ping before:absolute before:h-3 before:w-3 before:rounded-full before:content-[""] before:bg-foreground'
                              : ''
                          }`}
                        />
                      }
