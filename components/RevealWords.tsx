...
export default function RevealWords({
  ...
  return (
    <motion.div
      ...
      className={`relative base overflow-hidden whitespace-nowrap text-4xl md:text-8xl font-black uppercase ${
        invert ? 'text-background text-stroke' : ''
      }`}
      style={{
        lineHeight: 0.75,
      }}
    >
      <AnimatePresence mode='wait'>
        <motion.div className='flex' layout>
          <motion.h1
            className={`text-transparent tracking-widest ${
              invert ? 'no-text-stroke' : ''
            }`}
            layout
          >
