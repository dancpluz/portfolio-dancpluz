...
export default function Header({
  ...
  return (
    <nav className='fixed z-20 left-0 top-0 w-screen flex flex-col gap-6 md:flex-row justify-between py-6 px-4 md:px-40 bg-background/30 backdrop-blur-xs items-center'>
      <Link href='#inicio'>
        <Image
          alt={'Logo Lumentosh'}
          src={'logo.svg'}
    ...
    <li className={`flex relative w-full items-center`}>
      {icon && (
        <Link
          ...
          className={`-left-8 absolute transition-opacity rounded-full`}
          href={'#' + id}
        >
          <motion.p
            style={{ opacity: hovered ? 1 : textOpacity }}
            className='absolute font-normal transition-opacity left-1/2 transform -translate-x-1/2 translate-y-7 lg:-translate-y-5 text-xs'
          >
            {id.toUpperCase()}
          </motion.p>
      ...
      <motion.div
        style={{ width: scale }}
        className='relative z-10 w-3 h-0.5 bg-foreground'
      >
